 ...
 ** Author: Ryan Shin 
 ** Date: 2/04/18
 ** Description: This is a unit test for the outpost card in dominion.c
 *********************************************************************/
...
int main(){
    ...
    //set position 5 card to outpost
    post.hand[testPlayer][5] = outpost;
    ...
    testFlag += myAssert ("Player has 1 less card", &failFlag, (post.handCount[testPlayer] - pre.handCount[testPlayer])== -1);

    testFlag += myAssert ("Played Pile has 1 extra card", &failFlag, (post.playedCardCount - pre.playedCardCount == 1));

    testFlag += myAssert ("Player deck has same number of cards", &failFlag, pre.deckCount[testPlayer] - post.deckCount[testPlayer] == 0 );
    
    testFlag += myAssert ("Outpost flag is set", &failFlag, (post.outpostPlayed != 0));
